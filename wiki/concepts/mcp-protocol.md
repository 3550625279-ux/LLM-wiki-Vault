...
related:
  ...
  - "extended_by::[[sources/mineru-mcp-debugging-session]]"
---
...
- **stdio 通道极其脆弱**：MCP 用 stdin/stdout 通信，stdout 只能有 JSON-RPC 消息。任何意外的 print()、日志输出、进度条都会污染通道，导致 Claude Code 解析失败（`Invalid JSON: expected value at line 1 column 1`）。正确做法：用 `contextlib.redirect_stdout(sys.stderr)` 把非协议输出重定向到 stderr
- **MCP 服务器修改后必须重启**：MCP 是独立子进程，代码修改不会自动生效。需要通过 `/mcp` 重启或杀掉旧进程让 Claude Code 自动重建
- **僵尸进程风险**：多次 MCP 调用失败后，进程可能未被清理，占用 GPU 显存和文件锁。调试时先检查 `Get-Process` 清理残留

---

*首次记录: 2026-06-13 | 更新: 2026-06-15 | 来源: [[sources/mineru-mcp-debugging-session]]*
