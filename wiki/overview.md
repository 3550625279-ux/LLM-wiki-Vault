...
**核心问题**: 神经网络架构的演进脉络是什么？
**覆盖范围**:
- 经典架构: CNN (LeNet)、RNN、GRU、LSTM
- 注意力机制: 加性注意力、缩放点积注意力、多头注意力
...
**关键洞察**:
- CNN 用局部连接+权值共享压缩参数；RNN 用隐状态传递记忆
- 注意力机制的本质：动态的、内容相关的加权平均
- GRU/LSTM 通过门控机制解决 RNN 的梯度消失
- 从 NW 核回归到 √d 缩放：Softmax 死亡饱和区是一脉相承的问题
...
| 领域 | concept 节点 | 关键进展 |
|------|-------------|---------|
| Foundations | 29 | 双曲几何 + MC Dropout + 样本空间 + 随机变量 + 前推测度 + 随机向量 + 协方差 + 矩 + 信息熵 + 交叉熵 + KL散度 + SGD噪声泛化 + **参数估计六件套**（点估计/矩估计/MLE/顺序统计量/估计量评价/区间估计）+ **样本向量几何五件套**（样本向量/均值投影/中心化解耦/方差几何/自由度）+ **CLT归一化六件套**（CLT/√n来源/标准差L2/BatchNorm/LayerNorm/√d缩放） |
| Architecture | 2 | LLM 嵌入几何 + CAT 注意力 |
| Training | 16 | LoRA + HypLoRA + SetFit + 负面增强 + 对比学习 + 后训练四层（SFT/CoT/PRM/GRPO） + 数据飞轮 + Aha Moment |
| Alignment | 2 | RLHF + 奖励Hacking |
| Inference | 1 | Test-Time Compute（Self-Consistency/Best-of-N/MCTS） |
| Multimodal | 4 | CAGC: MIR + CAT + Bank + GCCL |
| Agents | 3 | 意图检测 + OOS 检测 + 自适应 ICL |
| Engineering | 6 | MCP 协议 + 不确定性路由 + 推理引擎 + CUDA/DLL |

**总节点数**: 82 (concept: 55, entity: 2, insight: 5, source: 12, comparison: 2, operation: 1, question: 2)
**Common Computer 领域节点**: 8（6 concept + 1 insight + 1 source，独立于 AI 领域）

---

*最后更新: 2026-06-16 | 节点总数: 82 | 下次更新触发: 10次 ingest 后*
