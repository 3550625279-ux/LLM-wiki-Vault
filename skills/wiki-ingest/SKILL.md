...
12. **Update dashboard.** Refresh `wiki/meta/dashboard.md`: system status, todo progress, monthly goal counts.
13. **Raw source link.** The source summary page in `wiki/sources/` MUST include a clickable link to the raw file at the top:
    ```markdown
